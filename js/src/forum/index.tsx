...
function isProseMirror(ed: any): boolean {
  return !!(ed && ed.view && ed.state && !ed.editor);
}

function isRichEditor(ed: any): boolean {
  return isTiptap(ed) || isProseMirror(ed);
}

function getSelectedText(node: any, ed: any, start: number, end: number): string {
  if (node) return node.value.slice(start, end);
  if (isTiptap(ed)) return ed?.editor?.state?.doc?.textBetween(start, end, ' ') ?? '';
  if (isProseMirror(ed)) return ed?.view?.state?.doc?.textBetween(start, end, ' ') ?? '';
  return '';
}

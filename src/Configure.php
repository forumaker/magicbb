...
class Configure
{
    ...
    public function __invoke(Configurator $config): void
    {
        ...
        if ($s('bb_hide_login')) {
            $config->BBCodes->addCustom('[login]{TEXT}[/login]', '<LOGIN><xsl:apply-templates/></LOGIN>');
        }

        if ($s('bb_hide_like')) {
            $config->BBCodes->addCustom('[like]{TEXT}[/like]', '<LIKE><xsl:apply-templates/></LIKE>');
        }

        if ($s('bb_hide_reply')) {
            $config->BBCodes->addCustom('[reply]{TEXT}[/reply]', '<REPLY><xsl:apply-templates/></REPLY>');
        }
    ...
    protected function addAnchorBBCodes(Configurator $config): void
    {
        ...
        $config->BBCodes->addCustom(
            ...
            '<a href="#magicbb-{@jump}" class="bb-jump"><xsl:apply-templates/></a>'
        );
    }
}
